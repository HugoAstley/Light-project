...
for(i in 1:length(ICD_9_code_groups_psychiatric)){disease_model_DOC_psychiatric[[i]] <- lmer(scale(ICD_9_code_groups_psychiatric[[i]]$DOC) ~ scale(ICD_9_code_groups_psychiatric[[i]]$mean_solar_noon) + scale(ICD_9_code_groups_psychiatric[[i]]$AGE) + (1|ICD_9_code_groups_psychiatric[[i]]$SEX) + (1|ICD_9_code_groups_psychiatric[[i]]$REGION),
...
colnames(coefficients.se.values_disease_model_DOC_psychiatric) <- c("Diagnoses", "Intercept", "SE", "Age_coefficient", "Intercept1", "Noon_coefficient", "Age_coefficient1")
...
####
coefficients_for_plot_positive <- coefficients_for_plot[coefficients_for_plot$Noon_coefficient > "0",]

ggplot(data= coefficients_for_plot_positive, aes(x = factor(reorder(coefficients_for_plot_positive$Diagnoses, -coefficients_for_plot_positive$Noon_coefficient)), y = coefficients_for_plot_positive$Noon_coefficient, ymin = (coefficients_for_plot_positive$Noon_coefficient-coefficients_for_plot_positive$confidence), ymax = (coefficients_for_plot_positive$Noon_coefficient+coefficients_for_plot_positive$confidence), colour = coefficients_for_plot_positive$Noon_coefficient > 0))+
  scale_colour_manual(name = 'PC1 > 0', values = setNames(c('skyblue','grey'),c(T, F)))+
  geom_errorbar()+
  geom_point()+
  geom_text(aes(label = paste(round(coefficients_for_plot_positive$Noon_coefficient, digits=4))), hjust = -0.2)+
...
coefficients_for_plot_negative_significant <- coefficients_for_plot[(coefficients_for_plot$Noon_coefficient+coefficients_for_plot$confidence) < "0",]

ggplot(coefficients_for_plot_negative_significant, aes(x = factor(reorder(coefficients_for_plot_negative_significant$Diagnoses, -coefficients_for_plot_negative_significant$Noon_coefficient)), y = coefficients_for_plot_negative_significant$Noon_coefficient, ymin = (coefficients_for_plot_negative_significant$Noon_coefficient-coefficients_for_plot_negative_significant$confidence), ymax = (coefficients_for_plot_negative_significant$Noon_coefficient+coefficients_for_plot_negative_significant$confidence), colour = coefficients_for_plot_negative_significant$Noon_coefficient > 0))+
  scale_colour_manual(name = 'PC1 > 0', values = setNames(c('skyblue','grey'),c(T, F)))+
  geom_errorbar()+
  geom_point()+
  geom_text(aes(label = paste(round(coefficients_for_plot_negative_significant$Noon_coefficient, digits=4))), hjust = -0.2)+
  geom_hline(yintercept= 0, linetype="dashed", colour="skyblue")+
  scale_x_discrete(name= "")+
  scale_y_continuous(name= "Correlation coefficient with light duration")+
  theme(panel.background = element_blank(),
...
coefficients_for_plot_negative <- coefficients_for_plot[coefficients_for_plot$Noon_coefficient < "0",]
coefficients_for_plot_negative_notsignificant <- coefficients_for_plot_negative[(coefficients_for_plot_negative$Noon_coefficient+coefficients_for_plot_negative$confidence) >= "0",]

ggplot(coefficients_for_plot_negative_notsignificant, aes(x = factor(reorder(coefficients_for_plot_negative_notsignificant$Diagnoses, -coefficients_for_plot_negative_notsignificant$Noon_coefficient)), y = coefficients_for_plot_negative_notsignificant$Noon_coefficient, ymin = (coefficients_for_plot_negative_notsignificant$Noon_coefficient-coefficients_for_plot_negative_notsignificant$confidence), ymax = (coefficients_for_plot_negative_notsignificant$Noon_coefficient+coefficients_for_plot_negative_notsignificant$confidence), colour = coefficients_for_plot_negative_notsignificant$Noon_coefficient > 0))+
  scale_colour_manual(name = 'PC1 > 0', values = setNames(c('skyblue','grey'),c(T, F)))+
  geom_errorbar()+
  geom_point()+
  geom_text(aes(label = paste(round(coefficients_for_plot_negative_notsignificant$Noon_coefficient, digits=4))), hjust = -0.2)+
  geom_hline(yintercept= 0, linetype="dashed", colour="skyblue")+
  scale_x_discrete(name= "")+
  scale_y_continuous(name= "Correlation coefficient with light duration")+
  theme(panel.background = element_blank(),
